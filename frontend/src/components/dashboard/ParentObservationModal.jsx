...
import { X, ArrowRight, ArrowLeft, Check, Shield, Sparkles, Zap } from 'lucide-react';
import { useProfile } from '../../context/ProfileContext';
import { useAudio } from '../../context/AudioContext';
import { createEmptyParentFeedback } from '@ai/parentFeedbackModel';
import { calculateParentObservationSignals } from '@ai/crossSignalIntelligence';
        ...
        <div style={{ padding: '0.9rem 1.4rem', overflowY: 'auto', flex: 1, display: 'flex', flexDirection: 'column', gap: '1.1rem' }}>
          ...
          {currentStep === 5 && (
            <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
              ...
              {/* 🎯 Recommendation Preview — Live preview of what will change for the child */}
              {(() => {
                const signals = calculateParentObservationSignals(formData);
                const hasAny = Object.values(signals).some(v => v !== null);
                if (!hasAny) return null;

                // Determine recommended activity from parent signals alone
                let previewActivityId = null;
                let previewIcon = '🌟';
                let previewTitle = '';
                let previewPrompt = '';
                let previewColor = '#4F46E5';
                let previewBg = '#EEF2FF';

                if (signals.tracing === 'needs_support') {
                  previewActivityId = 'letter-tracing';
                  previewIcon = '✍️';
                  previewTitle = isBengali ? 'বর্ণ অনুশীলন শুরু হবে' : 'Letter Tracing Practice';
                  previewPrompt = isBengali
                    ? 'বর্ণ লেখা ও আঁকার উপর বিশেষ মনোযোগ দেওয়া হবে'
                    : 'Extra focus on letter formation and stroke practice.';
                  previewColor = '#DB2777';
                  previewBg = '#FDF2F8';
                } else if (signals.speech === 'needs_support') {
                  previewActivityId = 'word-snapper';
                  previewIcon = '🔊';
                  previewTitle = isBengali ? 'ধ্বনি ও শব্দ অনুশীলন' : 'Phonics & Sound Practice';
                  previewPrompt = isBengali
                    ? 'বর্ণের শব্দ ও ধ্বনি মেলানোর কাজ দেওয়া হবে'
                    : 'Letter sounds, blending, and phonics activities will be suggested.';
                  previewColor = '#059669';
                  previewBg = '#ECFDF5';
                } else if (signals.reading === 'needs_support') {
                  previewActivityId = 'word-snapper';
                  previewIcon = '📖';
                  previewTitle = isBengali ? 'পড়া ও শব্দ গড়ার অনুশীলন' : 'Reading & Word Building';
                  previewPrompt = isBengali
                    ? 'শব্দ চেনা ও পড়ার মজার খেলা দেওয়া হবে'
                    : 'Guided reading and sight-word builder games will be recommended.';
                  previewColor = '#4F46E5';
                  previewBg = '#EEF2FF';
                } else {
                  previewActivityId = 'abc-fill-in';
                  previewIcon = '🚀';
                  previewTitle = isBengali ? 'চালিয়ে যাও!' : 'Great Progress!';
                  previewPrompt = isBengali
                    ? 'বাড়িতে ভালো অগ্রগতি দেখা যাচ্ছে — মজার খেলা চলতে থাকবে'
                    : 'Home observations look positive — fun challenge games will continue.';
                  previewColor = '#D97706';
                  previewBg = '#FFFBEB';
                }
                ...
                return (
                  <div
                    style={{
                      ...
                      border: `1.5px solid ${previewColor}30`,
                      borderRadius: '16px',
                  ...
                  >
                    <span style={{ fontSize: '1.6rem', lineHeight: 1 }}>{previewIcon}</span>
                    <div style={{ flex: 1 }}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: '0.4rem', marginBottom: '0.25rem' }}>
                        <Zap size={13} color={previewColor} />
                        <span style={{ fontSize: '0.7rem', fontWeight: 800, color: previewColor, textTransform: 'uppercase', letterSpacing: '0.04em' }}>
                          {isBengali ? 'শিশুর পরবর্তী কার্যক্রম পরিবর্তন হবে' : "Child's Recommended Activity Will Update"}
                        </span>
                      </div>
                      <div style={{ fontSize: '0.88rem', fontWeight: 800, color: '#1E293B', marginBottom: '0.2rem' }}>
                        {previewTitle}
                      </div>

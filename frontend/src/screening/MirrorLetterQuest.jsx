...
import { Volume2, CheckCircle, ArrowRight, RotateCcw, Sparkles, Play, AlertCircle } from 'lucide-react';
...
export default function MirrorLetterQuest({ onCompleteQuest }) {
  ...
  const startTracing = (e) => {
    e.preventDefault();
    if (isDemonstrating || status === 'completed_step') return;
  ...
  const drawTracing = (e) => {
    if (!isTracingDrawing || isDemonstrating || status === 'completed_step') return;
    e.preventDefault();
  ...
  const stopTracing = () => {
    ...
    if (currentStrokeRef.current.length > 0 && currentQ.dots) {
      const newStrokes = [...drawnStrokes, currentStrokeRef.current];
      setDrawnStrokes(newStrokes);

      const targetConfig = {
        char: currentQ.targetLetter || 'b',
        guideDots: currentQ.dots,
        bellyOrientation: (currentQ.targetLetter === 'b' ? 'right' : (currentQ.targetLetter === 'd' ? 'left' : null))
      };

      const validation = validateTracingAttempt({
        drawnStrokes: newStrokes,
        guideDots: currentQ.dots,
        collectedDotIds: tracingCollected,
      ...
      } else if (validation.reason !== 'no_strokes' && validation.reason !== 'too_short') {
